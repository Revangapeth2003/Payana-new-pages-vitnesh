...
const ChatBot = ({ 
  ...
  const [waitingForTextInput, setWaitingForTextInput] = useState(false);
  ...
  useEffect(() => {
    ...
    newSocket.on('show-options', (options) => {
      ...
      setWaitingForTextInput(false); // Disable text input when options are shown
    });
    ...
    newSocket.on('fetch-message', async (data) => {
      ...
      try {
        ...
        if (messageData && messageData.message && typeof messageData.message === 'string' && messageData.message.trim()) {
          ...
          // Enable text input for certain prompts that expect user input
          const textInputPrompts = ['prompts.name', 'prompts.nameSuccess', 'prompts.age'];
          if (textInputPrompts.includes(path)) {
  ...
  // Option click handler - Updated to work with backend
  ...
  const handleTypingStart = useCallback(() => {
    if (!isTyping && isConnected && socket && waitingForTextInput) {
  ...
  }, [conversationId, userId, isTyping, isConnected, socket, waitingForTextInput]);
  ...
  }, [handleTypingStart, waitingForTextInput]);
  ...
  const isInputDisabled = () => {
    return !isConnected || processingStatus.isProcessing || (showOptions.length > 0 && !waitingForTextInput);
  };

  const getPlaceholderText = () => {
    if (processingStatus.isProcessing) return processingStatus.message;
    if (showOptions.length > 0 && !waitingForTextInput) return "Please select an option above...";
    if (!isConnected) return "Connecting to PayanaOverseas...";
    if (waitingForTextInput) return "Type your response here...";
  ...
  if (isMinimized) {
    return (
    <div className="chatbot-launcher" onClick={toggleMinimize}>
      <div className="robot-container">
        <div className="robot-head">
          <div className="robot-antenna left-antenna">
            <div className="antenna-ball"></div>
          </div>
          <div className="robot-antenna right-antenna">
            <div className="antenna-ball"></div>
          </div>
      ...
      <div className="launcher-tooltip">
        <div className="tooltip-text">Hi! Need help?</div>
        <div className="tooltip-subtext">Chat with PayanaOverseas</div>
      </div>
      <div className={`connection-pulse ${isConnected ? 'online' : 'offline'}`}></div>
    </div>
  );
  }
          ...
          <div className="input-field-wrapper">
            ...
            <div className="input-actions">
              ...
              <button 
                ...
                disabled={isInputDisabled() || (!inputMessage.trim() && waitingForTextInput)}
